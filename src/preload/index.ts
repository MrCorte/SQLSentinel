  ...
  return {
    ...
    databases: [
      ...
      },
      {
        ...
        isReadOnly: false,
        owner: 'sa',
        createDate: '2019-06-01T00:00:00.000Z'
      },
      {
        name: 'OfflineDB',
        stateDesc: 'OFFLINE',
        ...
        isReadOnly: false,
        owner: 'dbo',
        createDate: '2020-03-15T00:00:00.000Z'
      }
    ],
    activeSessions: [
      {
    ...
    waitStats: [
      ...
      },
      {
        ...
        waitTimeMs: 28900,
        maxWaitTimeMs: 4100,
        signalWaitTimeMs: 890,
        waitingTasksCount: 12,
        waitPercent: 24.6
      },
      {
        waitType: 'CXPACKET',
...
const realApi = {
  ...
  workerStop: (): Promise<IpcResult<null>> => ipcRenderer.invoke(IpcChannel.WORKER_STOP),
  ...
  getAlerts: (): Promise<IpcResult<Alert[]>> => ipcRenderer.invoke(IpcChannel.ALERTS_GET_ALL),
  ...
  onMetricsUpdated: (
    ...
    const listener = (
      ...
      data: { serverId: string; metrics: ServerMetrics }
    ) => callback(data)
  ...
  sendTestEmail: (): Promise<IpcResult<null>> => ipcRenderer.invoke(IpcChannel.EMAIL_TEST),
  ...
  exportAlerts: (): Promise<IpcResult<string>> => ipcRenderer.invoke(IpcChannel.EXPORT_ALERTS),
  ...
  servers: {
    getAll: (): Promise<StoredServer[]> => ipcRenderer.invoke(IpcChannel.SERVERS_GET_ALL),
  ...
  logout: (): Promise<{ success: boolean }> => ipcRenderer.invoke(IpcChannel.AUTH_LOGOUT),
...
const mockApi = {
  scanSubnet: (_options: ScanOptions): Promise<IpcResult<DiscoveredServer[]>> =>
    new Promise((resolve) => setTimeout(() => resolve({ ok: true, data: MOCK_SERVERS }), 3000)),
    ...
    new Promise((resolve) =>
      setTimeout(
        () =>
          resolve({
            ok: true,
            data: { machineName: req.ip.replace(/\./g, '-'), instanceName: null }
          }),
        800
      )
    ),
  ...
  workerStart: (req: WorkerStartRequest): Promise<IpcResult<null>> => {
    ...
    mockWorkerTimer = setInterval(() => {
      _log.info(
        ...
        `listeners: ${mockMetricsListeners.length}`
      )
      ...
      for (const srv of req.servers) {
        ...
        _log.info(
          '[MockWorker] pushing metrics to',
          sid,
          `cpu=${metrics.instanceInfo.cpuUsagePercent.toFixed(1)}%`
        )
  ...
  onServerHealthUpdate:
    (_callback: (health: ServerHealthPayload) => void): (() => void) =>
    () => {},
  ...
  sendTestEmail: (): Promise<IpcResult<null>> => Promise.resolve({ ok: true, data: null }),
  ...
  exportInventory: (): Promise<IpcResult<string>> =>
    Promise.resolve({
      ok: true,
      data: 'ip,porta,raggiungibile,responseTimeMs,discoveredAt,database\r\n'
    }),

  exportAlerts: (): Promise<IpcResult<string>> =>
    Promise.resolve({
      ok: true,
      data: 'id,serverId,categoria,severita,messaggio,rilevato_il,acknowledged_il\r\n'
    }),
      ...
      new Promise((resolve) =>
        setTimeout(
          () =>
            resolve({
              ok: true,
              data: { success: true, duration_ms: 1800, newSizeMb: 12, reclaimedMb: 52 }
            }),
          2000
        )
      )
  ...
  onServerConfigUpdated:
    (_callback: (servers: StoredServer[]) => void): (() => void) =>
    () => {},
